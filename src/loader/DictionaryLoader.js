...
DictionaryLoader.prototype.load = function (load_callback) {
    ...
    var joinPath = function(base, ...paths) {
        let finalPath = base.endsWith('/') ? base : base + '/';
        paths = paths.map(path => path.startsWith('/') ? path.substr(1) : path);
    ...
    async.parallel([
        ...
        function (callback) {
            async.map([ "base.dat.gz", "check.dat.gz" ], function (filename, _callback) {
                loadArrayBuffer(joinPath(dic_path, filename), function (err, buffer) {
                    if(err) {
        ...
        function (callback) {
            loadArrayBuffer(joinPath(dic_path, "cc.dat.gz"), function (err, buffer) {
                if(err) {
                    return callback(err);
                }
